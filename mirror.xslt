...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:junos="http://xml.juniper.net/junos/*/junos" xmlns:xnm="http://xml.juniper.net/xnm/1.1/xnm" xmlns:ext="http://xmlsoft.org/XSLT/namespace" xmlns:jcs="http://xml.juniper.net/junos/commit-scripts/1.0" xmlns:func="http://exslt.org/functions" xmlns:dyn="http://exslt.org/dynamic" xmlns:local="http://xml.juniper.net/local" extension-element-prefixes="dyn func">
  ...
  <!-- This is a simple script that sets up mirroring of all GPIM ports to port  -->
  ...
  <xsl:template match="/">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$action = 'enable'">
        ...
        <xsl:value-of select="local:pfe-push($connection,concat('set jbcm command &quot; Mirror Mode=L2 Port=hg0',
          'IngressBitMap=ge0-ge23',
          'EgressBitMap=ge0-ge23',' &quot;'))"/>

        <xsl:value-of select="local:pfe-push($connection,'set jbcm slot 6')"/>
        <xsl:value-of select="local:pfe-push($connection,concat('set jbcm command &quot; Mirror Mode=L2 Port=ge22',
          'IngressBitMap=ge0-ge21,ge23,hg0,hg1',
          'EgressBitMap=ge0-ge21,ge23',' &quot;'))"/>

        <xsl:value-of select="local:pfe-push($connection,'set jbcm slot 0')"/>
        <xsl:value-of select="local:pfe-push($connection,concat('set jbcm command &quot; Mirror Mode=L2 Port=hg2',
          'IngressBitMap=hg0,hg1',
           '&quot;'))"/>
